layui.define(['jquery', 'jqutil'], function (exports) {
    ...
    tree.prototype.template = function (e) {
        ...
        if (e.isList){
            ...
            data_ = t.data(e, e.data);
        }
        o.each(data_, function (idx, item) {
            var tr = '',
                is_checked = false,
            ...
            o.each(e.cols, function (index, obj) {
                ...
                // 拼接行
                //构建view  确保view的id值不相同  采用列index+时间戳+【0-0】的一个随机数
                var tmp = new Date().getTime(), viewId = "view" + index + jqutil.genID(32);
                var view = '<div id=' + viewId + ' ></div>'
                tr += '<td ' + style + (left ? 'data-down' : '') + '>' + icon + (is_table ? '' : (is_checked ? checked : checkbox)) + (typeof obj.template == "function" ? obj.template(item) : obj.template == null ? (item[obj.key] == null || item[obj.key] == undefined) ? '' : item[obj.key] : obj.template == undefined ? item[obj.key] : view) + '</td>';
                if (typeof obj.template != "function" && obj.template != null && obj.template != undefined) {
                    var map = {};
                    map.laytplid = obj.template;
                    map.viewid = viewId;
                    map.data = item;
                    laytpls.push(map);
            ...
            var innterItem = {};
            innterItem.index = item[e.primary_key];
            innterItem.data = item;
            innerData.push(innterItem)
        });
        if (e.data == null || e.data == undefined || e.data.length < 1){
            //构建thead
            o.each(e.cols, function (index, obj) {
                var style = '';
                obj.width && (style += 'width:' + obj.width + ';'), obj.align && (style += 'text-align:' + obj.align + ';'), style && (style = 'style="' + style + '"');
            ...
            var colspan = e.cols.length + 1;
            tbody += '<tr><td style="text-align: center" colspan=\''+colspan+'\'><p>暂无数据</p></td></tr>';
        }
        // 处理表树和树的赋值模板
        ...
        o(e.elem).html(table).off('click', click_btn).on('click', click_btn, function () {
            ...
            var is_checked = o(this).toggleClass('layui-form-checked').hasClass('layui-form-checked'),
                ...
                id = tr.data('id'),
                pid = tr.data('pid');
            t.childs_checkbox111(e, id, is_checked);
        ...
        //初始化模板
        if (laytpls.length > 0) {
            for (var i = 0; i < laytpls.length; i++) {
                var getTpl = document.getElementById(laytpls[i].laytplid).innerHTML
                    , view = document.getElementById(laytpls[i].viewid);
                laytpl(getTpl).render(laytpls[i].data, function (html) {
                    view.innerHTML = html;
                });
            }
        }
        e.innerData_ = innerData;
    };
    ...
    // 子级反选
    tree.prototype.childs_checkbox111 = function (e, id, is_checked) {
        var t = this;
        o(e.elem).find('[data-pid=' + id + ']').each(function () {
            ...
            t.childs_checkbox111(e, o(this).data('id'), is_checked);
        })
    };
    ...
    tree.prototype.reload = function (e, params) {
        if (params != null && params != undefined) {
            e.params = params.where;
        }
        this.render(e);
    }
    ...
    tree.prototype.checkStatus = function (e, index) {
        ...
        o(e.elem).find('tr').each(function () {
            //获取选中行
            if (o(this).find('td').eq(index).find('.layui-form-checked').length) {
                var index_ = o(this).data('id');
                for (var i = 0; i < innerData.length; i++) {
                    if (innerData[i].index == index_) {
                        data.push(innerData[i].data);
                    }
                }
            }
        });
